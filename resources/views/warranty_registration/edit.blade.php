...
<div class="modal-body ms-5">
    ...
    <form method="POST" action="{{ route('warrentyRegistration.update',$warrent_registration->id) }}" accept-charset="UTF-8"
        ...
        <div class="row">
            <div class="form-group col-md-6">
              ...
              <input type='text'name='warranty_no' value='{{$warrent_registration->warranty_no}}' class='form-control'/>
            </div>
            <div class="form-group col-md-6">
                ...
                <select id='work_order_id' class='form-control ' name="work_order_id">
                    ...
                    <option value="{{$work->id}}" {{$wo_id = $work->id ? 'selected' : ''}}> WO-{{$wo_id }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                {{ Form::text('customer_name',  $warrent_registration->customer->first_name, ['class' => 'form-control basic-select','id'=>'customer_name','required'=>'required','readonly'=>true]) }}
            ...
            <div class="form-group col-md-6">
                ...
                <select id='product' class='form-control basic-select' name="product_id" required>
                    <option value="{{$warrent_registration->product_id}}">{{$warrent_registration->product?->product_name}}</option>
                </select>


            </div>
            <input type="hidden" id="customer_id" name="customer_id" value="{{$warrent_registration->customer_id}}">

            <div class="form-group col-md-6">
                {{ Form::label('vehicle_id', __('Vehicle'), ['class' => 'form-label']) }}


                {{ Form::text('vehicle_id', 'VCL'.$warrent_registration->vehicle, ['class' => 'form-control basic-select','required'=>'required','id'=>'vehicle_name','readonly'=>true]) }}

            </div>

            <div class="form-group col-md-6">
                {{ Form::label('invoice_id', __('Invoice ID'), ['class' => 'form-label']) }}
             <input type="text" id="customer_id" name="customer_id" value="{{$wo_id}}" class='form-control'>
            </div>
            <div class="form-group col-md-6">
            ...
            <div class="form-group col-md-6">
                ...
                {{ Form::text('warranty_period', $warrent_registration->warranty_period, ['class' => 'form-control basic-select','required'=>'required','id'=>'w_period']) }}
            </div>
            <div class="form-group col-md-6">
                {{ Form::label('warranty_start_date', __('Warranty Start Date'), ['class' => 'form-label']) }}

                {{Form::date('warranty_start_date',\Carbon\Carbon::parse($warrent_registration->warranty_start_date),array('class'=>'form-control','required'=>'required','id'=>'start'))}}
            </div>
            <div class="form-group col-md-6">
                {{ Form::label('warranty_end_date', __('Warranty End Date'), ['class' => 'form-label']) }}

                {{Form::text('warranty_end_date',$warrent_registration->warranty_end_date,array('class'=>'form-control','required'=>'required','readonly'=>true,'id'=>'end'))}}
            </div>
            <div class="form-group col-md-6">
                {{ Form::label('status', __('Status'), ['class' => 'form-label']) }}

                <select class='form-control basic-select' name="status" required>
                    <option value='1' {{$warrent_registration->status = 1 ? 'selected' : ''}}>Active</option>
                    <option value='2'{{$warrent_registration->status = 2 ? 'selected' : ''}}>Already Claimed</option>
                </select>
