...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto"> {{ __('Brand') }}</h4><span
        ...
        <div class="d-flex my-xl-auto right-content">
            ...
            <div class="mb-3 mb-xl-0">
                <div class="btn-group dropdown">
                    @if (Gate::check('create brand'))
                        <a class="btn btn-primary customModal" href="#" data-size="lg"
                            data-url="{{ route('brand.create') }}" data-title="{{ __('Add Brand') }}">
                            <i class="ti-plus"></i>
                            {{ __('Create Brand') }}
                        </a>
                    @endif
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table text-md-nowrap datatbl-advance">
                            ...
                            <tbody>
								@foreach ($brand->sortByDesc('id')->values() as $key => $item)
                                    <tr>
										<td class="d-none"></td>
                                        <td width="20" class="text-center">B-{{ $item->id }} </td>
                                        <td>{{ $item->description }} </td>
                                        <td>
                                            <div class="cart-action">
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['brand.destroy', $item->id]]) !!}
                                                @can('edit brand')
