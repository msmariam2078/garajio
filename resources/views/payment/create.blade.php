        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <input value="" type="hidden" id="workorder_id" />

                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="customerOrderNumber">Post Date</label>
                            <input type="text" class="form-control" id="customerOrderNumber"
                                placeholder="Enter customer order number" value="{{\Carbon\Carbon::today()}}" readonly>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="customerOrderNumber">Work Order</label>
                            <select type="text" class="form-control" onchange="availableInvoice()" id="workorder">
                            <option>Select Work Order</option>    
                            @foreach($workorders as $workorder)
                                
                                <option value="{{$workorder->id}}">#WO-{{$workorder->id}}</option>
                               @endforeach
                ...
                <div class="d-flex justify-content-end mt-12">
                    <button type="button" onclick="submitPayment()" class="btn ml-2" style="background:blue;color:white">Pay Now</button>
...
function availableInvoice(){
    var workOrder = parseInt(document.getElementById("workorder").value);
    const tablebody = document.getElementById("tbody");
    tablebody.innerHTML="";
    $.ajax({
        url: '/workorder/invoice?workorder='+ workOrder,
        method: 'get',
        success(response){
      
        if(response.message=="success")
        {
        tablebody.innerHTML = `<tr onclick="selectInvoice(this)">
          <td>${response.data.invoice_id}
                        <input type="hidden" class="invoice-id" value="${response.data.id}" />
             </td>
        <td>${response.data.invoice_date}</td>
      
        <td>${response.data.total}</td>
         <td>${response.data.discount}</td>
        <td>${response.data.final_amount}</td>
      
        </tr>
    `;
        }
        else{
            tablebody.innerHTML ='<p style="text-align:center;font-size:20px">no invoice found</p>' ;  
        }
        } 
    });
}
...
function updateSummary() {
    ...
    document.getElementById("amount").value = appliedTotal.toFixed(2);
    checkSubmitStatus();
}
...
function submitPayment() {
    ...
    const workOrderId = parseInt(document.getElementById("workorder").value);
        ...
        success: function(response) {
            if (response.success) {
                ...
                Swal.fire({
                  ...
                  text: "INVOICE NO."+ response.invoiceId +" "+response.message,
                   icon: "success",
                       draggable: true
                })
                .then((willDelete) => {
...
function addPaymentMethod() {
    ...
    let totalBalanceDue = selectedInvoices.reduce((sum, invoice) => sum + (invoice.balanceDue - invoice.appliedAmount),
        0);
...
function distributePayment(amount) {
    ...
    for (let i = 0; i < selectedInvoices.length && amount > 0; i++) {
        ...
        if (availableBalance > 0) {
            ...
            document.getElementById("amount").value =`${invoice.balance.toFixed(2)}`;
        }
    }
...
function removePayment(index) {
  ...
  //console.log(payments[index].amount);
    ...
    // let newsubtotal = parseInt(document.getElementById("subtotal").textContent) - payments[index].amount;
    // let newtotal = parseInt(document.getElementById("total").textContent) + payments[index].amount;
    ...
    //   document.getElementById("subtotal").textContent =document.getElementById("total").textContent + payments[index].amount;
    //  document.getElementById("total").textContent +=payments[index].amount;
